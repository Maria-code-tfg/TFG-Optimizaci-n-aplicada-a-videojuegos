...
# ALGORITMOS PARA OBTENER CIRCUNFERENCIAS
...
def Circunferencia_de_puntos_mas_separados(poligono: list[Punto])->Circunferencia:
    """
    Obtiene la circunferencia que tiene como centro el punto medio del AABB que encierra
    al polígono.
    """
    ...
    return Circunferencia(centro, sqrt(radio2))

def Circunferencia_de_circunferencia_y_punto(s: Circunferencia, p: Punto)->Circunferencia:
    """
    Dada una circunferencia y un punto, si el punto no está en el interior de la circunferencia, genera
    una nueva circunferencia a partir de la que ya se tenía para que encierre también al punto p.
    """
    d = Vector(p.x-s.centro.x, p.y-s.centro.y)
    dist2 = d.prod_escalar(d)
    if not s.punto_dentro(p):
        dist = sqrt(dist2)
...
def Circunferencia_Ritter(poligono: list[Punto])->Circunferencia:
    num_puntos = len(poligono)
    s = Circunferencia_de_puntos_mas_separados(poligono)
    for i in range(num_puntos):
        s = Circunferencia_de_circunferencia_y_punto(s, poligono[i])
    return s
...
def EigenCircunferencia(poligono: list[Punto]):
...
def Circunferencia_Ritter_PCA(poligono: list[Punto]):
    s = EigenCircunferencia(poligono)
    n = len(poligono)
    for i in range(n):
        s = Circunferencia_de_circunferencia_y_punto(s, poligono[i])
    return s
...
# Funciones auxiliares para definir circunferencias a partir de puntos (de 0 a 3)

def circunferencia_cero_puntos():
    return Circunferencia(Punto(0,0),-1)

def circunferencia_un_punto(p:Punto):
    return Circunferencia(p,0)

def circunferencia_dos_puntos(p:Punto, q:Punto):
    centro = Punto((p.x+q.x)/2,(p.y+q.y)/2)
    radio = sqrt(((p.x-q.x)**2+(p.y-q.y)**2))/2
    return Circunferencia(centro,radio)

def circunferencia_tres_puntos(p:Punto,q:Punto,r:Punto):
    x1, y1 = p.x, p.y
    x2, y2 = q.x, q.y
    x3, y3 = r.x, r.y
    ...
    if abs(D) < ERROR:
        ...
        candidatos = [
            circunferencia_dos_puntos(p, q),
            circunferencia_dos_puntos(p, r),
            circunferencia_dos_puntos(q, r)
        ]
        ...
        return mejor  
    # Se calcula el circuncentro del triángulo que definen los tres puntos.
...
# Función para obtener la circunferencia más pequeña que contiene al polígono

def Circunferencia_Welzl(poligono:list[Punto], num_puntos: int, sos: list[Punto], num_sos: int):
    if num_puntos == 0 or num_sos == 3:
        if num_sos==0:
            return circunferencia_cero_puntos()
        elif num_sos==1:
            return circunferencia_un_punto(sos[0])
        elif num_sos==2:
            return circunferencia_dos_puntos(sos[0], sos[1])
        elif num_sos==3:
            return circunferencia_tres_puntos(sos[0], sos[1], sos[2])
    else:
        i = num_puntos-1
        circunferencia_pequeña = Circunferencia_Welzl(poligono, num_puntos-1, sos, num_sos)
        if (circunferencia_pequeña.punto_dentro(poligono[i])):
            return circunferencia_pequeña
        else:
            sos[num_sos] = poligono[i]
            return Circunferencia_Welzl(poligono, num_puntos-1, sos, num_sos+1)
        
def obtener_circunferencia(poligono: list[Punto]):
    sos = [None]*3
    num_puntos = len(poligono)
    circunferencia = Circunferencia_Welzl(poligono,num_puntos,sos,0)
    return circunferencia
